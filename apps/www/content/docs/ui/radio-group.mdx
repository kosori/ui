---
title: Radio Group
description: A set of checkable buttons—known as radio buttons—where no more than one of the buttons can be checked at a time.
links:
  doc: https://www.radix-ui.com/docs/primitives/components/radio-group
  api: https://www.radix-ui.com/docs/primitives/components/radio-group#api-reference
dependencies: ['@radix-ui/react-radio-group']
---

<ComponentPreview name='radio-group'>
```json doc-gen:file
{
  "file": "./src/components/demos/radio-group/radio-group.tsx",
  "codeblock": true
}
```
</ComponentPreview>

## Installation

<InstallTabs items={['CLI', 'Manual']}>

  <Tab value='CLI'>
  
    <KosoriCLITabs>
      ```bash tab="npm"
      npx @kosori/cli add components radio-group
      ```

      ```bash tab="pnpm"
      pnpx @kosori/cli add components radio-group
      ```

      ```bash tab="yarn"
      yarn dlx @kosori/cli add components radio-group
      ```

      ```bash tab="bun"
      bun x @kosori/cli add components radio-group
      ```

    </KosoriCLITabs>

  </Tab>

  <Tab value='Manual'>

    <Steps>

      <Step>
        ### Install the primitive

        Install the `@radix-ui/react-radio-group` package.

        ```package-install
        @radix-ui/react-radio-group
        ```

      </Step>

      <Step>
        ### Copy-paste the component

        Copy and paste the component code in a `.tsx` file.

        ```json doc-gen:file
        {
          "file": "../../packages/ui/src/radio-group.tsx",
          "codeblock": true
        }
        ```

      </Step>

      <Step>
        ### Update import paths

        Update the `@kosori/ui` import paths to fit your project structure, for example, using `~/components/ui`.

      </Step>

    </Steps>

  </Tab>

</InstallTabs>

## Usage

```ts
import { Label } from '~/components/ui/label';
import { RadioGroup, RadioGroupItem } from '~/components/ui/radio-group';
```

```tsx
<RadioGroup defaultValue='option-one'>
  <div className='flex items-center space-x-2'>
    <RadioGroupItem value='option-one' id='option-one' />
    <Label htmlFor='option-one'>Option One</Label>
  </div>
  <div className='flex items-center space-x-2'>
    <RadioGroupItem value='option-two' id='option-two' />
    <Label htmlFor='option-two'>Option Two</Label>
  </div>
</RadioGroup>
```

## Examples

### Form

<ComponentPreview name='radio-group-form'>
```json doc-gen:file
{
  "file": "./src/components/demos/radio-group/radio-group-form.tsx",
  "codeblock": true
}
```
</ComponentPreview>
