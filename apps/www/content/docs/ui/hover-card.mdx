...
```json doc-gen:file
{
  "file": "./src/components/demos/hover-card/hover-card.tsx",
  "codeblock": true
}
```
</ComponentPreview>
...
<InstallTabs items={['CLI', 'Manual']}>
  ...
  <Tab value='CLI'>
    ...
    <KosoriCLITabs>
      ```bash tab="npm"
      npx @kosori/cli add components hover-card
      ```

      ```bash tab="pnpm"
      pnpx @kosori/cli add components hover-card
      ```

      ```bash tab="yarn"
      yarn dlx @kosori/cli add components hover-card
      ```

      ```bash tab="bun"
      bun x @kosori/cli add components hover-card
      ```
